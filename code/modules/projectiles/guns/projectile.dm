...
// Код предохранителя
/obj/item/weapon/gun/projectile/Fire(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, params, reflex = 0)
	if(!safety)
		user << "<span class='warning'>You can't shot, because [src] safety on!"
		return
	else if(safety == 2)
		for(var/i = 0, i < 3, i++)
			..(target, user, params, reflex)
	else
		..(target, user, params, reflex)

// Кликай для победы.
/obj/item/weapon/gun/projectile/CtrlClick(mob/living/user)
	if(!(src in user.contents))
		..(user)
	switch(safety)
		if(0)
			user << "<span class='warning'>Safety off!"
			safety = 1
		if(1)
			if(!automatic)
				user << "<span class='warning'>Safety on."
				safety = 0
			else
				user << "<span class='warning'>Auto mode."
				safety = 2
		if(2)
			user << "<span class='warning'>Safety on."
			safety = 0